...
void setup()
{
  ...
  RGB_sensor.config(CFG1_MODE_RGB | CFG1_375LUX, CFG2_IR_ADJUST_HIGH, CFG_DEFAULT | CFG3_INT_PRST2);
  ...
  RGB_sensor.setUpperThreshold(0x0000);
...
void loop()
{
  ...
  double red_value = 0; // Stores sensor reading for red light intensity
  double green_value = 0; // Stores sensor reading for red light intensity
  double blue_value = 0; // Stores sensor reading for red light intensity
  ...
  if (lasti != i)
  {
    ...
    Serial.println(i);
    Serial.print("Red Sensor Value (DEC): ");
    Serial.println(red_value, DEC);
    Serial.print("Green Sensor Value (DEC): ");
    Serial.println(green_value, DEC);
    Serial.print("Blue Sensor Value (DEC): ");
